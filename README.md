...
```json
{
  ...
  "SPOTIFY_CACHED_TOKEN": "anything can be put here, as it is overridden with the cached token",
  "SPOTIFY_CACHE_EXPIRATION": 0
}
```

8. Replace `./icons.json` (the app icons cache) with {}, it will build a cache on the first run, after that you can edit it if any icons aren't being detected to replace `class` with the `window.initialClass` or change any icons
