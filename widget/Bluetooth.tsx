...
const [bluetoothDevices, setBluetoothDevices] = createState<BtDevice[]>([])
const [bluetoothScanning, setBluetoothScanning] = createState(false)

function updateBluetoothDevices() {
  setBluetoothDevices([])
  execAsync(["sh", "-c", "bluetoothctl devices | sed 's/\\x1b\\[[0-9;]*m//g'"])
    .then((d) =>
      d
        .split("\n")
        .map((line) => {
        ...
        .filter(
          ({ name, mac }) =>
            name && mac && !/^([0-9A-F]{2}[-:]){5}[0-9A-F]{2}$/i.test(name),
        ),
    )
...
export function BluetoothButton({ gdkmonitor }: { gdkmonitor: Gdk.Monitor }) {
  function toggleBluetooth() {
    if (activePopup() == "bluetooth") {
      setActivePopup(null)
    } else {
      setBluetoothMargin(centeredMargin(bluetoothButtonRef, gdkmonitor))
    ...
    >
      ...
      <centerbox class="title">
        ...
        <box $type="end">
          ...
          >
            <label
              label={bluetoothScanning((v) => (v ? "scanning" : "scan"))}
            />
          </button>
          <button
            $={(self) =>
          ...
          <For each={bluetoothDevices}>
            {(device: BtDevice) => (
              <button
                ...
                onClicked={() => execAsync(`bluetoothctl connect ${device.mac}`).catch(() => {})}
              >
                <box>
                  <label
                    class="icon"
                  ...
                  />
                  <label halign={Gtk.Align.CENTER} label={device.name} />
                </box>
              </button>
            )}
          </For>
