...
>({})
const windowIsSelected = createComputed(() => !!selectedWindow().class)
...
export function ArchPopup({ gdkmonitor }: { gdkmonitor: Gdk.Monitor }) {
  return (
    <Popup
    ...
    >
      ...
      <button
        $={(self) => self.set_cursor(Gdk.Cursor.new_from_name("pointer", null))}
        onClicked={() =>
          execAsync('notify-send "System Settings" "this isn\'t completed yet"')
        }
      >
      ...
      <button
        ...
        $={(self) => self.set_cursor(Gdk.Cursor.new_from_name("pointer", null))}
        onClicked={() => execAsync(`pkill -15 -p ${selectedWindow().pid}`)}
      >
      ...
      <button
        visible={windowIsSelected}
        $={(self) => self.set_cursor(Gdk.Cursor.new_from_name("pointer", null))}
        onClicked={() => execAsync(`pkill -9 -p ${selectedWindow().pid}`)}
      >
